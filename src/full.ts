...
class Peer extends Duplex<any, any, any, any, true, true, PeerEvents> {
    ...
    constructor (opts:any = {}) {
        ...
        if (!RTCPeerConnection) {
            if (typeof window === 'undefined') {
                throw errCode(
                    new Error('No WebRTC support: Specify `opts.wrtc` ' +
        ...
        this._queuedNegotiation = false  // is there a queued negotiation
        ...
        // We prefer feature detection whenever possible, but sometimes
        // that's not possible for certain implementations.
    ...
    signal (data:any) {
        ...
        if (data.sdp) {
            this._pc.setRemoteDescription(new RTCSessionDescription(data))
                ...
                .catch(err => {
                    this.__destroy(
                        errCode(err, 'ERR_SET_REMOTE_DESCRIPTION')
                    )
                })
        }
    ...
    _addIceCandidate (candidate:any) {
        ...
        this._pc.addIceCandidate(iceCandidateObj)
            .catch(err => {
                const address = (iceCandidateObj as any).address
                if (!address || address.endsWith('.local')) {
                    warn('Ignoring unsupported ICE candidate.')
                } else {
                    this.__destroy(
                        errCode(err as Error, 'ERR_ADD_ICE_CANDIDATE')
                    )
                }
            })
    }
   ...
   */
    ...
    _setupData (event:any) {
        if (!event.channel) {
            ...
            return this.__destroy(errCode(
                new Error('Data channel event is missing `channel` ' +
                    'property'),
                'ERR_DATA_CHANNEL'
            ))
        }
